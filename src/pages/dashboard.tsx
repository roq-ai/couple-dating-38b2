...
import { getComplaints } from 'apiSdk/complaints';
import { ComplaintInterface } from 'interfaces/complaint';
import { getConnectionOptions } from 'apiSdk/connection-options';
...
function DashboardPage() {
  ...
  const complaintStat = useSWR<PaginatedInterface<ComplaintInterface>>(
    () => `/complaints`,
    () => getComplaints({ limit: 1, offset: 0 }),
  );

  const connectionOptionStat = useSWR<PaginatedInterface<ConnectionOptionInterface>>(
    () => `/connection-options`,
    () => getConnectionOptions({ limit: 1, offset: 0 }),
  );
  ...
  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    ...
    {
      title: 'Complaints',
      result: complaintStat?.data?.totalCount,
      isLoading: complaintStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'Connection Options',
      result: connectionOptionStat?.data?.totalCount,
      isLoading: connectionOptionStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
